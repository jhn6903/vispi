...
try:
    ...
    while True:
        ...
        if os.system('read -n 1 -t 0.01') == 0:
            char = os.popen('read -n 1 -s && echo $REPLY').read().strip()
            if char == 'p':
        ...
        is_silent = np.max(np.abs(samples)) < config.settings['noise_gate']
        ...
        if is_silent:
            ...
            fft = np.clip(fft * config.settings['sensitivity'], 0, 1)
        ...
        for _ in range(int(random.randint(10, 20) * config.settings['intensity'])):
        ...
        for _ in range(int(energy * 300 * config.settings['intensity'])):
            x = random.randint(0, WIDTH - 1)
            y = random.randint(0, HEIGHT - 1)
            hue = (time.time() * config.settings['color_speed'] + x * 0.01) % 1.0
            color = hsv_to_rgb(hue, 1, random.uniform(0.5, 1))
            draw.point((x, y), fill=color)

        # Occasional colored stripe
